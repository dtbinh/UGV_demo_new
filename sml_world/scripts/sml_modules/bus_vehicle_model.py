...
class BusVehicle(DummyVehicle):
	def __init__(self, namespace, vehicle_id, simulation_rate,
		...
		#If start location not set, just put at first place
		if x == 0 and y == 0:
	...
	def simulation_step(self):
		...
		if self.at_dest:
			...
			#Slow down to "pick up passengers"
			rospy.sleep(WAIT_AT_BUS_STATION_DURATION)
			self.add_demand_to_model(self.cur_destination, -60)
			setattr(req, 'speed', 50)
			self.handle_set_speed_kph(req)
			if not self.stops:
				self.request_new_stops()
	...
	def request_new_stops(self):
		'''
		Sends a request for rerouting based on current location
		'''
		...
		try:
			...
			current_node = get_nodeid(self.x, self.y, -10, True).node_id
