...
import rospy
import os, pygame
import TrafficLightManager

from sml_world.msg import LaneletTraffic
from sml_world.srv import GetCrosswalkNodes, GetCrosswalkNodesResponse
from sml_world.srv import GetCrosswalkIds, GetCrosswalkIdsResponse
...
class RoadModule(object):
    ...
    def __init__(self, base_path, file_path, image_width = 1920., image_height = 1080., image_pixel_per_meter = 3.2):
        ...
        # Store the traffic light nodes seperate
        self.traffic_light_nodes = None

        # Store the crosswalks
        self.osm_crosswalk_dict = None
        ...
        rospy.Service('/get_crosswalk_nodes', GetCrosswalkNodes, self.handle_get_crosswalk_nodes)
        rospy.Service('/get_crosswalk_ids', GetCrosswalkIds, self.handle_get_crosswalk_ids)
    ...
    def load_xml_world_info(self, xml_file_location):
        ...
        [self.osm_node_dict, self.osm_node_tag_dict, origin_osm_node,
                self.traffic_light_nodes, self.crosswalk_dict] = RoadModuleFileReader.create_osm_node_dict(
                                                                            xml_file_location)

        self.normalize_node_coordinates(origin_osm_node)

        self.osm_way_dict = RoadModuleFileReader. \
                create_osm_way_dict(xml_file_location)

        # List of nodes makes sense
        self.osm_lanelet_dict = RoadModuleFileReader.create_osm_lanelet_dict(
                self.osm_node_dict, self.osm_way_dict, xml_file_location)

        self.lanelet_adjacency_matrix = RoadLibrary. \
                create_lanelet_adjacency_matrix(self.osm_lanelet_dict,
                self.osm_node_dict)

        # Stores amount of traffic on each of the lanelets
        self.traffic_status = [1 for i in range(
            len(self.lanelet_adjacency_matrix))]

        RoadDrawer.set_pixel_values_for_nodes(self,
            self.desired_image_width, self.desired_image_height,
            self.desired_image_pixel_per_meter)

        return

    def start_traffic_light_manager(self):
        TrafficLightManager.TrafficLightManager(self.traffic_light_nodes)

    def handle_get_crosswalk_ids(self, req):
        """
        @author: U{Nicolo' Campolongo<nicoloc@kth.se>}
        Handle the get crosswalk id request.
        """
        l = []
        for i in self.crosswalk_dict:
            l.append(i)
        return GetCrosswalkIdsResponse(l)

    def handle_get_crosswalk_nodes(self, req):
        """
        @author: U{Nicolo' Campolongo<nicoloc@kth.se>}
        Handle the get crosswalk request.
        @param req: I{(GetCrosswalk)} Request of the service that gets the
                    id of the 2 crosswalk nodes given the crosswalk_id.
        """
        node_1 = self.crosswalk_dict[req.crosswalk_id][0]
        node_2 = self.crosswalk_dict[req.crosswalk_id][1]
        return GetCrosswalkNodesResponse(node_1, node_2)
    ...
    def get_open_path(self, start_osm_node_id, end_osm_node_id, points_per_meter = 5):
        ...
        for lanelet_id in containing_lanelet_ids:
            ...
            lanelet_ids_path = RoadDijkstra.lanelet_dijkstra_algorithm(self,
                            start_lanelet_id, end_lanelet_id, self.traffic_status)
    ...
    def get_closed_path_from_node_id(self, osm_node_id, points_per_meter = 5):
        ...
        for lanelet_id in containing_lanelet_ids:
            ...
            lanelet_ids_path = RoadDijkstra.lanelet_circular_dijkstra_algorithm(
                self, lanelet_id, self.traffic_status)
    ...
    def get_path_between_node_ids(self, start_osm_node_id, end_osm_node_id, points_per_meter = 5):
        ...
        for start_lanelet_id in start_lanelet_ids:
            for end_lanelet_id in end_lanelet_ids:
                if not circle:
                    ...
                    lanelet_ids_path = RoadDijkstra. \
                        lanelet_circular_dijkstra_algorithm(self, start_lanelet_id,
                                    self.traffic_status)
        ...
        if not best_lanelets_path:
            # No path found, so we return a length which is infinite
    ...
    def update_lanelet_traffic(self, lanelet_data):
        '''
        The traffic ratio is defined as the following equation in
        (5 + TRAFFIC_LEVEL) / 5
        For example, if the traffic level is 10, the car will travel
        1/3 as slowly on that lanelet, and we should take that into
        account
        '''
        self.traffic_status[lanelet_data.lanelet] = (5. + lanelet_data.new_traffic_level) / 5.
