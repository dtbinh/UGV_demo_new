...
from sml_world.msg import WiFiData
...
class BaseVehicle(WheeledVehicle):
    ...
    def simulation_step(self):
        ...
        if self.vehicle_id == 3:
            ref_ind = (closest_ind + 40) # closest_ind + numpy.round(self.v / 4)
        else:
            ref_ind = closest_ind + numpy.round(self.v / 3)
    ...
    def set_control_commands(self, ref_state, ref_ind):
        ...
        if self.overtake:
            ...
            dy_v = -numpy.sin(self.yaw) * dx + numpy.cos(self.yaw) * dy + 3.5
        else:
...
class DummyVehicle(BaseVehicle):
    ...
    def check_if_overtake_is_finished(self):
        ...
        if not numpy.any(self.radar_readings[0, :]):
            ...
            if self.overtake_counter > 40 and self.overtake:
                self.overtake = False
                desired_speed  = self.cruising_speed
        else:
            self.overtake_counter = 0
...
class TruckVehicle(ConnectivityVehicle):
    ...
    def __init__(self, namespace, vehicle_id, simulation_rate,
        ...
        # To get qualisys data
        
        self.truck_id = self.mocap.get_id_from_name("MiniTruck3") #("MiniTruck3") "MiniTruck"+str(self.vehicle_id)
        ...
        rospy.Subscriber('/WiFi_data', WiFiData, self.UDP_data_handle)
       
                                      # -1.00 corresponds to no physical obstacle

    # Receive physical obstacle data    
    ...
    def UDP_data_handle(self, data):
        self.data = str(data.data)
        #rospy.logwarn(data)

    # Check for physical obstacles. If true, low level control. Else, high level control.
    def set_control_commands(self, ref_state, ref_ind):

        self.update_vehicle_state_qualisys()
        #self.UDP_receive()
        #rospy.logwarn(self.data)
        
        if self.data == "-1.00":
            self.set_control_commands_pp(ref_state, ref_ind)
        else:
            #rospy.logwarn("NOT -1")
            #rospy.logwarn(" DATA :" + str(self.data))
            steer = int(self.data[-6:-3])
            throttle = int(self.data[:-6])
            hw_port.set_command(throttle,steer,self.vehicle_id)
            rospy.logwarn("throttle : " + str(throttle) + "    steer : " + str(steer) )
    ...
    def set_control_commands_pp(self, ref_state, ref_ind):
        ...
        if numpy.abs(steering_command) < 0.05:
            self.commands['steering_angle'] = 0

        safety_distance = 12.
        full_stop_distance = 7.
    ...
    def update_vehicle_state_qualisys(self):
        ...
        if truck_state == 'off':
            ...
            # Transformation from Qualisys coordinates to SML world coordinates.
    ...
    def update_vehicle_state(self):
        ...
        # Low level controller takes effect if obstacle detected.
        if self.data != "-1.00":
            return
